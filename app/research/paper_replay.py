...
def _price_at_bin(connection: sqlite3.Connection, pool_address: str, bin_id: int, observed_at: float) -> float | None:
...
def _drain_score(connection: sqlite3.Connection, pool_address: str, bin_id: int, observed_at: float) -> float | None:
    row = connection.execute(
...
def build_replay_points(connection: sqlite3.Connection, *, position_address: str, min_fee_velocity_sol_min: float = 0.0) -> tuple[ReplayPoint, ...]:
    ...
    rows = connection.execute(
        ...
        FROM position_analytics WHERE position_address = ?
    ...
    for row in rows:
        ...
        if active_bin is None or lower_bin is None or upper_bin is None or in_range is None or fee_sol is None:
            continue
        ...
        points.append(ReplayPoint(
            position_address=position_address, pool_address=pool, observed_at=observed_at,
            price=price, fee_delta_sol=fee_delta, fee_velocity_sol_min=velocity,
            drain_score=drain_score, in_range=bool(in_range),
            lower_price=min(lower_price, upper_price), upper_price=max(lower_price, upper_price),
        ))
        previous_at = observed_at
    return tuple(points)


def _historical_canonical_state(connection: sqlite3.Connection, position_address: str, observed_at: float) -> CanonicalPositionState | None:
    return load_canonical_position_state(connection, position_address, observed_at=observed_at)


def _authoritative_fee_total(connection: sqlite3.Connection, position_address: str, entry_at: float, exit_at: float) -> float | None:
    rows = connection.execute(
        """SELECT fee_sol, reset_or_claim FROM position_analytics
        WHERE position_address = ? AND observed_at >= ? AND observed_at <= ?
        ORDER BY observed_at ASC, id ASC""",
...
def replay_position(connection: sqlite3.Connection, *, position_address: str, deposit_sol: float = 1.0,
                    out_of_range_seconds: int = 20, max_drain_score: float = 0.95,
                    min_fee_velocity_sol_min: float = 0.0, min_score: float = 0.7) -> ReplayResult:
    points = build_replay_points(connection, position_address=position_address, min_fee_velocity_sol_min=min_fee_velocity_sol_min)
    if not points:
    ...
    for point in points:
        ...
        if actual_state is None:
            ...
            continue
        signal = evaluate_canonical_state(actual_state, point.fee_velocity_sol_min,
                                          min_fee_velocity=min_fee_velocity_sol_min,
                                          max_drain=max_drain_score, min_score=min_score)
        if not opened:
            if signal.entry:
                engine.open(position_address, point.pool_address, point.price, point.lower_price, point.upper_price,
                            deposit_sol, timestamp=point.observed_at, x_amount=actual_state.x_amount,
    ...
    if entry_at is not None and exit_at is not None and entry_state is not None:
        ...
        if current_state is not None and fees_sol is not None:
            try:
                ...
                pnl = None
    return ReplayResult(position_address, points[0].pool_address, tuple(points), tuple(engine.events), tuple(skipped), pnl)


def replay_all_positions(connection: sqlite3.Connection, *, deposit_sol: float = 1.0,
                         out_of_range_seconds: int = 20, max_drain_score: float = 0.95,
                         min_fee_velocity_sol_min: float = 0.0, min_score: float = 0.7) -> tuple[ReplayResult, ...]:
    rows = connection.execute("SELECT DISTINCT position_address FROM position_analytics ORDER BY position_address").fetchall()
    return tuple(replay_position(connection, position_address=row[0], deposit_sol=deposit_sol,
                                  out_of_range_seconds=out_of_range_seconds, max_drain_score=max_drain_score,
                                  min_fee_velocity_sol_min=min_fee_velocity_sol_min, min_score=min_score) for row in rows)


def persist_replay_result(connection: sqlite3.Connection, result: ReplayResult, *,
                          strategy_key: str = "canonical-heart-attack-replay") -> bool:
    """Persist a closed replay only when its PnL and fee ledger are authoritative."""
    ...
    exit_events = [event for event in result.events if event["action"] in {"OUT_OF_RANGE", "DRAIN_EXIT"}]
    if not open_events or not exit_events:
        return False
    opened, exited = open_events[0], exit_events[-1]
    entry_at, exit_at = float(opened["timestamp"]), float(exited["timestamp"])
    metadata = json.dumps({"position_address": result.position_address, "source": "authoritative_sqlite_replay"}, sort_keys=True)
    ensure_paper_trades(connection)
    exists = connection.execute(
        "SELECT 1 FROM paper_trades WHERE strategy_key = ? AND pool_address = ? AND entry_at = ? AND exit_at = ? LIMIT 1",
        (strategy_key, result.pool_address, entry_at, exit_at),
    ).fetchone()
    if exists is not None:
        return False
    fees = result.dlmm_pnl.fees_sol
    gross = result.dlmm_pnl.net_pnl_sol - fees
    record_trade(connection, pool_address=result.pool_address, strategy_key=strategy_key,
                 entry_at=entry_at, exit_at=exit_at, entry_price=float(opened["price"]),
                 exit_price=float(exited["price"]), gross_pnl_sol=gross, fees_sol=fees,
                 net_pnl_sol=result.dlmm_pnl.net_pnl_sol, exit_reason=exited["action"],
                 metadata_json=metadata)
    return True


def persist_replay_results(connection: sqlite3.Connection, results: tuple[ReplayResult, ...], *,
                           strategy_key: str = "canonical-heart-attack-replay") -> int:
