...
class PositionFeeDelta:
    ...
    def fee_x_per_minute(self) -> float:
        return self.delta_fee_x / self.elapsed_seconds * 60 if self.elapsed_seconds > 0 else 0.0

    @property
    def fee_y_per_minute(self) -> float:
        return self.delta_fee_y / self.elapsed_seconds * 60 if self.elapsed_seconds > 0 else 0.0
